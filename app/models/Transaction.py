...
class Transaction(Model):
    ...
    category_id = db.Column(
        db.UUID(as_uuid=True), db.ForeignKey("category.id"), nullable=False
    )
    invoice = db.Column(db.String(160), unique=False, nullable=False)
    payment_option_id = db.Column(
        db.UUID(as_uuid=True), db.ForeignKey("payment_option.id"), nullable=False
    )
    is_monthly = db.Column(db.Boolean, unique=False, nullable=False, default=False)
    user_id = db.relationship("User", backref="transaction", passive_deletes=True)
    ...
    def __repr__(self):
        return (
            f"<Transaction(id={self.id or 'N/A'}, "
            f"title='{self.title}', "
            f"location='{self.location}', "
            f"amount={self.amount}, "
            f"category_id={self.category_id}, "
            f"invoice='{self.invoice}', "
            f"payment_option_id={self.payment_option_id}, "
            f"is_monthly={self.is_monthly}, "
